...
class TestCLIScriptInvocation(unittest.TestCase):
    ...
    def test_pygubu_create_version(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'pygubuai.create', '--version'],
            capture_output=True,
            text=True
        )
    ...
    def test_pygubu_create_help(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'pygubuai.create', '--help'],
            capture_output=True,
            text=True
        )
    ...
    def test_pygubu_register_help(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'pygubuai.register', '--help'],
            capture_output=True,
            text=True
        )
    ...
    def test_pygubu_template_help(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'pygubuai.template', '--help'],
            capture_output=True,
            text=True
        )
        # Should work even without pygubu
    ...
    def test_all_entry_points_exist(self):
        ...
        for ep in entry_points:
            ...
            self.assertTrue(
                module_file.exists(),
                f"Module {module_file} not found for entry point {ep}"
            )
